...
private:
    ...
    void splitChild(BTreeNode* parentNode, int childIndex) {
        ...
        BTreeNode* split = new BTreeNode(child->isLeaf, t);

        int mid = t - 1;

        for (int i = 0; i < mid; ++i) {
            split->keys[i] = child->keys[i + t];
            split->values[i] = child->values[i + t];
        }
        
        if (!split->isLeaf) {
            for (int i = 0; i < t; ++i) {
                split->children[i] = child->children[i + t];
            }
        }
        ...
        parentNode->children[childIndex + 1] = split;
        ...
        parentNode->values[childIndex] = child->values[mid];
