 ...
 * @tparam Hash Hash Table function type. Defaults to hash<K>.
 */
...
private:
     ...
     */
    struct Slot {
        optional<pair<K, V>> data; // Holds the key-value pair if occupied
        SlotState state;                     // Current state of the slot

        // Constructor
        Slot() : data(nullopt), state(SlotState::EMPTY) {}
    };

    vector<Slot> table;    // Dynamic array to store slots
...
public:
     ...
     *
     * @return vector<K> A vector containing all keys.
     */
    vector<K> keys() const;
...
int main() {
    ...
    if (value) {
        cout << "\nFound key " << searchKey << " with value: " << *value << endl;
    } else {
    ...
    cout << "\nRemoving '" << removeKey << "': " << (removed ? "Successful" : "Not found") << endl;
